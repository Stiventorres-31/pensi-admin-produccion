...
const Service = () => {
    ...
    const openModal = (op, id, descrip) => {
        setOperation(op);
        if (op == 1) {
            setTitle('Crear tipo de servicio');
        } else {
    ...
    const deleteServicios = (id_, name) => {
        confirmation(name, '/api/servicios/' + id_, 'servicios');
    }
    ...
    const columns = [
        ...
        { name: 'id_tipo_servicio', label: 'ID', options: { filter: false, sort: true } },
                    ...
                    return (
                        <>
                            <div className='inline-flex justify-center space-x-4 w-full'>
                                ...
                                <button className="block text-white hover:bg-acent focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center bg-[#be0a0a]" onClick={() => deleteServicios(id, name)}><i className="fa-solid fa-trash"></i></button>
